...
test("revision review demo flow", async ({ page }) => {
  ...
  await expect(page.getByText("ORION EV / REVISIONS").first()).toBeVisible();

  // run analysis
  await page.getByRole("button", { name: "Analyze Revision" }).click();
  await expect(page.getByText(/changes · \d+/).first()).toBeVisible({ timeout: 60_000 });

  // click the flange change
  await page.getByText(/flange diameter/).first().click();
  await expect(page.getByText("deterministic checks").first()).toBeVisible();
  await expect(page.getByText(/ORN-FS-017 §4.2/).first()).toBeVisible();

  // dependencies page
  await page.getByRole("link", { name: "Dependencies" }).click();
  await expect(page.getByText(/store: (sql|neo4j)/)).toBeVisible();
  await expect(
    page.locator('.react-flow__node', { hasText: "Chassis Interface" }).first(),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "send" }).click();
  await expect(page.getByText("human confirmation required").first()).toBeVisible({ timeout: 60_000 });
  await page.getByRole("button", { name: /Confirm · create_review_finding/ }).click();
  await expect(page.getByText(/finding created/).first()).toBeVisible({ timeout: 30_000 });
